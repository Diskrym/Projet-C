    ...
    if (boss->Life >=1)
    {
        ...
        {   
            ...
            if (boss->CompteurSpriteEclair>=21 && boss->CompteurSpriteEclair<=30)
            {
                ...
                if (boss->CompteurSpriteEclair==22)
                {
                    Mix_VolumeChunk(son->attaquemeduse, MIX_MAX_VOLUME);
                    Mix_PlayChannel(1, son->attaquemeduse, 0);
                }
            }
            ...
            if (boss->CompteurSpriteEclair>=51 && boss->CompteurSpriteEclair<=60 )
            {
                ...
                if (boss->CompteurSpriteEclair==52)
                {
                    Mix_VolumeChunk(son->attaquemeduse, MIX_MAX_VOLUME);
                    Mix_PlayChannel(1, son->attaquemeduse, 0);
                }
            ...
            if (boss->CompteurSpriteEclair>=81 && boss->CompteurSpriteEclair<=90 )
            {
                ...
                if (boss->CompteurSpriteEclair==82)
                {
                    Mix_VolumeChunk(son->attaquemeduse, MIX_MAX_VOLUME);
                    Mix_PlayChannel(1, son->attaquemeduse, 0);
                }
            ...
            if (boss->CompteurSpriteEclair>=111 && boss->CompteurSpriteEclair<=121 )
            {
                ...
                if (boss->CompteurSpriteEclair==112)
                {
                    Mix_VolumeChunk(son->attaquemeduse, MIX_MAX_VOLUME);
                    Mix_PlayChannel(1, son->attaquemeduse, 0);
                }
...
{   //Verification pointeur pour mémoire
    ...
    if ((lvl->Num == 4 && lvl->WinDonjon<4) || (lvl->Num == 5 && lvl->WinDonjon<=2))
    {
...
void Sprite_Boss_Jerem(Joueur *joueur, Jerem_Boss *jerem, Lvl *lvl, EffetSon *son)
{
    ...
    //Degats sur jerem en fonction de son niveau de transformation
    ...
    if (jerem->Life >20 )
    {
        //Eattaque=0 correspond a la marche basique
        if(jerem->Eattaque == 0)
        {
            //Si il ne prend pas de degats
            ...
            else if (jerem->Direction == 0)
            {
                ...
                Draw_Image(jerem->jeremy,jerem->posmonsx,jerem->posmonsy);
                son->sondegatjerem++;
                if (son->sondegatjerem==1)
                {
                    Mix_VolumeChunk(son->degatjerem, MIX_MAX_VOLUME);
                    Mix_PlayChannel(14, son->degatjerem, 0);
                }  
            }
            else
            {
        ...
        //attaque haltère phase 1 (Eattaque == 1)
        if (jerem->Eattaque == 1)
        {
            if (jerem->Direction==1)
            {            
    ...
    else if (jerem->Life <=20 && jerem->Transformation == 0 && jerem->Life != 0)
    {
        ...
        if (jerem->Direction == 0)
        {
            ...
            {                
                ...
                Draw_Image(jerem->jeremy,jerem->posmonsx,jerem->posmonsy);
                if (jerem->CompteurSprite2==1)
                {
                    Mix_VolumeChunk(son->transformation, MIX_MAX_VOLUME);
                    Mix_PlayChannel(14, son->transformation, 0);
                } 
            }
        ...
        if (jerem->Eattaque == 4 || jerem->Eattaque == 5)
        {
            if(jerem->Direction == 0)
            {
                if (jerem->NumSprite>=30 && jerem->NumSprite<35)
                {
                    ...
                    if (jerem->NumSprite==30)
                    {
                        Mix_VolumeChunk(son->dash, MIX_MAX_VOLUME);
                        Mix_PlayChannel(14, son->dash, 0); 
                    }
                }
                if (jerem->NumSprite>=35 && jerem->NumSprite<40)
                {
    ...
    else if (jerem->Life == 0)
    {
        ...
        if (jerem->Direction == 0)
        {
            ...
            if (jerem->CompteurSprite2>=30 && jerem->CompteurSprite2<40)
            {
                ...
                Draw_Image(jerem->jeremy,jerem->posmonsx,jerem->posmonsy);
                if (jerem->CompteurSprite2==31)
                {
                    Mix_VolumeChunk(son->mortjerem, MIX_MAX_VOLUME);
                    Mix_PlayChannel(21, son->mortjerem, 0); 
                }
            }
            if (jerem->CompteurSprite2>=40 && jerem->CompteurSprite2<50)
            {
    ...
    if (jerem->Ehaltère == 1)
    {
        if (jerem->xhaltère>=20 && jerem->xhaltère<=600 && jerem->yhaltère>= 20 && jerem->yhaltère<=384)
        {
            if(jerem->Transformation == 2)
            {
                ...
                if (son->sonhaltere==1)
                {
                    Mix_VolumeChunk(son->attackJerem, MIX_MAX_VOLUME);
                    Mix_PlayChannel(14, son->attackJerem, 0);
                }
        ...
        else 
        {
            jerem->Ehaltère = 0;
            son->sonhaltere=0;
            Mix_VolumeChunk(son->haltere, MIX_MAX_VOLUME);
            Mix_PlayChannel(14, son->haltere, 0);
        }
    }
    if (jerem->coup != 0)
    {
